...
- 決定: コメントは created_at と messageId_ を保持する
...
- 未決: 添付ファイル本体の保存先は `Vercel Blob` と `Cloudflare R2` のどちらにするか
  - `Vercel Blob`: Vercel との統合が素直で実装が軽めです。一方で、配信やストレージ制御の自由度は R2 より少なめです。
  - `Cloudflare R2`: S3 互換で将来の移植性と配信制御の自由度が高いです。一方で、接続設定や権限制御は Blob より少し増えます。
...
- 決定理由: 現プロジェクトでは postgres を第一推奨とする。既存の repository / service 分離と最も相性がよく、ORM
  導入なしで保存先だけを Postgres に置き換えやすいため。 SQL を明示的に管理でき、今回採用する SQL ファイル migration
  方針とも整合する。 @neondatabase/serverless は将来 Edge runtime 等が必要になった場合の補助候補とする
...
- migration はいつ流すか:
  ...
  - [ ] deploy 後ジョブ
  - [x] その他
- その他: リリース前の手動実行。現段階では CI / Vercel build / deploy
- 後ジョブによる自動実行は採用せず、schema 変更を含むリリース前に担当者が共有環境へ明示的に migration
  を適用する。branch-per-preview を採用しないため、自動化
- すると失敗や適用順ミスの影響が複数環境に波及しやすい。手動実行のほうがトラブルを抑えやすく、運用も軽い。
...
- migration ツール
- 添付ファイル本体の保存先
- `DATABASE_URL` / `DATABASE_URL_UNPOOLED` の使い分け

### 次のアクション

1.  DB 接続ライブラリを確定する
2.  migration ツールと実行タイミングを確定する
3.  添付ファイル保存先を `Vercel Blob` と `Cloudflare R2` から確定する
4.  接続文字列の使い分けと権限分離方針を確定する
